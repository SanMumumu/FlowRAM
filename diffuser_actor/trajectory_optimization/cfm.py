...
class DiffuserActor(nn.Module):
    ...
    def conditional_sample(self, condition_data, condition_mask, fixed_inputs):
        ...
        for t in timesteps:
            out = self.policy_forward_pass(
                trajectory,
                t * torch.ones(len(trajectory)).to(trajectory.device),
                fixed_inputs
            )
    ...
    def compute_trajectory(
        ...
        # end_time = time.time()
        # print("Time for trajectory generation: ", end_time - start_time)
    ...
    def forward(
        ...
        for layer_pred in pred:
            ...
            trans_l = 30 * F.l1_loss(trans, target[..., :3], reduction='mean')
            rot_l = 10 * F.l1_loss(rot, target[..., 3:9], reduction='mean')
            trans_loss = trans_loss + trans_l
            rot_loss = rot_loss + rot_l

            if torch.numel(gt_openess) > 0:
                openess = layer_pred[..., 9:].mean(1, keepdim=True)
                loss = F.binary_cross_entropy_with_logits(openess, gt_openess)
                open_loss = open_loss + loss
        total_loss = trans_loss + rot_loss + open_loss
        return total_loss, trans_loss, rot_loss, open_loss
...
class DiffusionHead(nn.Module):
    ...
    def forward(self, trajectory, timestep,
        ...
        traj_feats = torch.permute(traj_feats, [1, 0, 2])
        context_feats = torch.permute(context_feats, [1, 0, 2])
        adaln_gripper_feats = torch.permute(adaln_gripper_feats, [1, 0, 2])
    ...
    def encode_denoising_timestep(self, timestep, curr_gripper_features):
        ...
        curr_gripper_features = torch.permute(curr_gripper_features, [1, 0, 2])
    ...
    def predict_pos(self, features, rel_pos, time_embs, num_gripper,
        ...
        position_features = torch.permute(position_features[:num_gripper], [1, 0, 2])
    ...
    def predict_rot(self, features, rel_pos, time_embs, num_gripper,
        ...
        rotation_features = torch.permute(rotation_features[:num_gripper], [1, 0, 2])
