...
class TrainTester(BaseTrainTester):
    ...
    def train_one_step(self, model, criterion, optimizer, step_id, sample):
        ...
        out, trans_loss, rot_loss, open_loss = model(
            sample["trajectory"],
        ...
        # Log
        if dist.get_rank() == 0:
            self.writer.add_scalar("lr", optimizer.param_groups[0]['lr'], step_id)
            self.writer.add_scalar("train-loss/trans_loss", trans_loss, step_id)
            self.writer.add_scalar("train-loss/rot_loss", rot_loss, step_id)
            self.writer.add_scalar("train-loss/open_loss", open_loss, step_id)
    ...
    def evaluate_nsteps(self, model, criterion, loader, step_id, val_iters,
        ...
        for i, sample in enumerate(loader):
            ...
            pos_l2 = losses['pos_l2_final']
            rot_l1 = losses['rot_l1']
            batch_size = len(action)
            rot_loss += rot_l1.item()*batch_size
            pos_loss += pos_l2.item()*batch_size
            size += batch_size

            if dist.get_rank() == 0:
                self.writer.add_scalar("val-loss/pos_l2_final", losses['pos_l2_final'], step_id)
                self.writer.add_scalar("val-loss/pos_l2_final<0.01", losses['pos_l2_final<0.01'], step_id)
                self.writer.add_scalar("val-loss/rot_l1", losses['rot_l1'], step_id)
                self.writer.add_scalar("val-loss/rot_l1<0.025", losses['rot_l1<0.025'], step_id)

        return pos_loss/size, rot_loss/size
...
class TrajectoryCriterion:
    ...
    def compute_metrics(pred, gt, mask):
        ...
        ret_1 = ({
            ...
            'rot_l1': quat_l1.mean(),
            'rot_l1<0.025': (quat_l1 < 0.025).float().mean()
        })
        ret_2 = ({
            'pos_l2_final': pos_l2,
